...
std::vector<glm::vec4>* ok::graphics::SpriteBatch::quad_extra_data = nullptr;

ok::graphics::SpriteBatch::SpriteBatch(int size, int extra_data_channels)
{
	batch_size = size;

	extra_data.resize(extra_data_channels);
...
void ok::graphics::SpriteBatch::Draw(ok::graphics::SpriteInfo * sprite_info, glm::vec2 position, float rotation_degrees, glm::vec2 scale)
{
	...
	{
		quad_extra_data = &(sprite_info->extra_data);
	}
	else
	{
		quad_extra_data = nullptr;
	}
...
void ok::graphics::SpriteBatch::BatchEnd()
{
	...
	//setup shader
	if (batch_texture != nullptr)
	current_material->SetTexture(0, batch_texture);
	else
	{
		if (current_material->GetTextureSlotsCount() != 0)
		{
			//prevent rendering
			LOG(ERROR) << "ERROR: Nullptr texture inside sprite batch, material texture slots count is non zero.";
			batch_quads_in_use = 0;
			return;
		}
	}
...
void ok::graphics::SpriteBatch::PushQuad(bool zero_color_data)
{
	...
	{
		ptr = &(extra_data[i][batch_quads_in_use * quad_extra_data_size]);
		auto vec4_data = glm::value_ptr((*quad_extra_data)[i]);

		for (int k = 0; k < 4; k++)
		{
			memcpy(ptr, vec4_data, sizeof(float) * quad_extra_data_size / 4);
			ptr += 4;
		}	
	}
